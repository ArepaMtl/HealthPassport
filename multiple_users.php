...
<html manifest="cache.manifest">
	...
	<head>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				var isUpdated = localStorage.getItem("history-is-updated-2-0");
				...
				if (isUpdated === null || isUpdated === false) {
					...
					for (var i = 0; i < localStorage.length; i++){
						...
						if (key.indexOf("history-") === 0 && key.indexOf("history-user:") !== 0) {
					...
					if (foundKeys) {
						...
						var userName = historyObject["history-user-name"];
						
						if (typeof userName === 'undefined' || userName === null || userName === false) {
							userName = "Default (click 'Edit' to change name)"
						}
						
						var newId = addUser(userName);
						
						for (var key in historyObject) {
							var realKey = "history-user:" + newId + "-" + key.substring("history-".length);
							
							localStorage.setItem(realKey,historyObject[key]);
						}
					...
					localStorage.setItem("history-is-updated-2-0",currentDate.toString());
  ...
  //then grab the encoded contents back out.  The div never exists on the page.
				...
				function addUserLink(name,id) {
					...
					var words = removeDiacritics(name).split(" ");
					
					var initials = ""
					
					for (var j=0; j<words.length; j+=1){
						if (words[j].length > 0 && initials.length < 2) {
							var newChar = words[j].substring(0,1).toUpperCase();
					...
					$("<li class='item multipleuserscolor user-item' id='user-item-"+id+"'> <a href=\"history.php#"+id+"\" class='user-link'>  <div class='item-text'> <div class='user-name-text'> "+htmlEncode(name)+" </div> </div> </a> <div class='noclick mulefticon multipleusers_icon' href='' style=\"background-color:"+allColors[colorNumber]+"\"><div>"+initials+"</div></div> <a class='remove-button righticon-invisible' href='' data-user-id='"+id+"'> </a> <input type='text' id='user-edit-box-"+id+"' data-edit-user-id='"+id+"' class='edit-name-textbox edit-invisible'/> <div class='noclick forward-button'> </div></li>").insertBefore("#addme-li");
					
					$("#user-edit-box-"+id).val(name);
				...
				function updateUserName(id,newName) {
					...
					localStorage.setItem("user-"+i,newName);
				...
				function startEditing(){
					...
					$(".user-name-text").addClass("edit-invisible");
					$(".edit-name-textbox").removeClass("edit-invisible");
				}
				...
				function stopEditing(){
					...
					$(".user-name-text").removeClass("edit-invisible");
					$(".edit-name-textbox").addClass("edit-invisible");
				}
				...
				function addUserActions(){
					...
					$( ".edit-name-textbox").unbind("input propertychange paste change");
					
					$(".edit-name-textbox").on("input propertychange paste change",function(){
						$(this).parent().find(".user-name-text").text($(this).val());
						
						var userId = $(this).attr("data-edit-user-id");
						
						updateUserName(userId,$(this).val());
